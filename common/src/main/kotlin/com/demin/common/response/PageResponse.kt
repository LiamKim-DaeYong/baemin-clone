package com.demin.common.response
...
data class PageResponse<T>(
    ...
    companion object {
        fun <T> success(content: List<T>, pageDetails: PageDetails, message: String? = null, httpStatus: Int = 200): PageResponse<T> {
            return PageResponse(Status.SUCCESS, message, content, pageDetails, httpStatus)
        }

        fun <T> fail(message: String? = null, httpStatus: Int = 400): PageResponse<T> {
            return PageResponse(Status.FAIL, message, emptyList(), PageDetails(0, 0, 0, 0), httpStatus)
        }

        fun <T> error(message: String, httpStatus: Int = 500): PageResponse<T> {
